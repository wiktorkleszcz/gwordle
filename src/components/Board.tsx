    ...
    function handleKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
        ...
        if (e.key === "Backspace") {
            ...
            if (board.letters[position.row][position.col]) {
                boardDispatch({ type: 'clearLetter', row: position.row, col: position.col })
                return
            }

            if (position.col > 0) {
                boardDispatch({ type: 'clearLetter', row: position.row, col: position.col - 1 })
                positionDispatch({ type: 'moveLeft', length })
                return
            }

            if (position.row > 0) {
                boardDispatch({ type: 'clearLetter', row: position.row - 1, col: length - 1 })
                positionDispatch({ type: 'moveLeft', length })
            }
