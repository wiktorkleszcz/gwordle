...
function createEmptyStatuses(tries: number, length: number): (LetterStatus | null)[][] {
    return Array.from({ length: tries }, () =>
...
function boardReducer(state: BoardState, action: BoardAction): BoardState {
    switch (action.type) {
        ...
        case 'submit': {
            ...
            const statuses = state.letters.map((row) => checkGuess(row, action.solution))
...
function positionReducer(state: Position, action: PositionAction): Position {
    switch (action.type) {
        ...
        case 'moveLeft':
            ...
            return state
        case 'moveRight':
            if (state.col < action.length - 1) return { ...state, col: state.col + 1 }
            if (state.row < action.tries - 1) return { row: state.row + 1, col: 0 }
            return state
        case 'moveUp':
            return state.row > 0 ? { ...state, row: state.row - 1 } : state
        case 'moveDown':
            return state.row < action.tries - 1 ? { ...state, row: state.row + 1 } : state
        case 'reset':
...
export function BoardProvider({ children }: { children: ReactNode }) {
    ...
    const [position, positionDispatch] = useReducer(positionReducer, initialPosition)
